...
def emit_firmware_geom(r, geom, path):
    """Emit a C header with the per-keycap board geometry the firmware needs to
    render the SAME effect procedurally: per-half disp_idx -> (board cx,cy, angle),
    the global board size, and the letter target positions. Board coords are in
    OLED-native-pixel units (each keycap pixel = 1 unit); the angle byte drives
    sin8/cos8. Everything is `const` -> flash only, no RAM."""
    import math
    # scale = preview board px per native OLED pixel (|ux| of an unrotated key)
    scales = [math.hypot(g["ux"], g["uy"]) for g in geom.values()]
    scales.sort()
    scale = scales[len(scales) // 2]                       # median (robust)
    W = int(round(r.cw / scale))
    H = int(round(r.ch / scale))

    def entry(left, dr, dc):
        mp = _disp_mp(left, dr, dc)
        g = geom.get(mp)
        if g is None:
            return "{0,0,0,0}"
        cx = int(round(g["cx"] / scale))
        cy = int(round(g["cy"] / scale))
        ang = int(round(math.degrees(math.atan2(g["uy"], g["ux"])) / 360.0 * 256)) & 0xFF
        return f"{{{cx},{cy},{ang},1}}"

    def table(left):
        rows = []
        for dr in range(5):
            rows.append("    " + ", ".join(entry(left, dr, dc) for dc in range(8)))
        return ",\n".join(rows)

    tg_for_letters = splash_targets()

    def letter_table(left):
        rows = []
        for dr in range(5):
            cps = [tg_for_letters.get(_disp_mp(left, dr, dc), "") for dc in range(8)]
            rows.append("    " + ", ".join(f"0x{ord(c):X}" if c else "0" for c in cps))
        return ",\n".join(rows)

    # letter targets (global board coords), all halves, for the spark converge
    tg = splash_targets()
    targets = []
    for mp, ch in tg.items():
        g = geom.get(mp)
        if g:
            targets.append((int(round(g["cx"] / scale)), int(round(g["cy"] / scale)), ord(ch)))

    lines = []
    lines.append("// Auto-generated by tools/startup_anim_demo.py --emit-geom. DO NOT EDIT.")
    lines.append("// Per-keycap board geometry for the procedural startup animation.")
    lines.append("#pragma once")
    lines.append("#include <stdint.h>")
    lines.append("")
    lines.append("typedef struct { int16_t cx, cy; uint8_t ang, valid; } sa_key_geom_t;")
    lines.append(f"#define SA_BOARD_W {W}")
    lines.append(f"#define SA_BOARD_H {H}")
    lines.append("")
    lines.append("// indexed by disp_idx = disp_row*8 + disp_col  (0..39)")
    lines.append("static const sa_key_geom_t SA_GEOM_LEFT[40] = {")
    lines.append(table(True) + "\n};")
    lines.append("static const sa_key_geom_t SA_GEOM_RIGHT[40] = {")
    lines.append(table(False) + "\n};")
    lines.append("")
    lines.append("// splash letter codepoint per disp_idx (0 = no letter on this key)")
    lines.append("static const uint16_t SA_LETTER_LEFT[40] = {")
    lines.append(letter_table(True) + "\n};")
    lines.append("static const uint16_t SA_LETTER_RIGHT[40] = {")
    lines.append(letter_table(False) + "\n};")
    lines.append("")
    # self-contained 8-bit sine table (angle 0..255 -> 0..255, centered 128), so
    # the firmware module needs no RGB-matrix lib8tion include path
    sintab = [int(round(128 + 127 * math.sin(2 * math.pi * i / 256))) for i in range(256)]
    sintab = [max(0, min(255, v)) for v in sintab]
    lines.append("static const uint8_t SA_SIN[256] = {")
    ...
    # (was 32) makes the repetition much less visible (more "randomized").
...
def main():
    ...
    ap.add_argument("--emit-geom", metavar="PATH",
                    help="write the firmware geometry header and exit")
    ...
    if args.emit_geom:
        emit_firmware_geom(r, geom, args.emit_geom)
        return
