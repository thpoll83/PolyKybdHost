...
# --------------------------------------------------------------------------- #
# Obsidian: an irregular faceted stone
# --------------------------------------------------------------------------- #
# The seed IS the design. Every stone in the family is generated by the same
# code and differs only by `seed`, so recording the number reproduces the chosen
# mark byte-for-byte — there is nothing hand-placed to preserve. 97 was picked
# from a rendered contact sheet of the family.
OBSIDIAN_SEED = 97


def _stone_facets(seed: int, nb: int, k: int, jitter: float, base: float,
                  swell: float, xs: float, ys: float, cy: float):
    """Facets of an irregular drop, in 0..1 cell coordinates.

    Every source of symmetry is deliberately broken: the apex is off-centre,
    each boundary point carries its own radial jitter, the interior seed points
    are scattered, and the boundary ring is cut into arcs of UNEQUAL length. An
    earlier revision jittered only the facet lines and kept a mirrored
    silhouette — it still read as symmetrical, which is the trap here.

    The radius swells with `sin(t)`, which peaks at the BOTTOM of the ring, and
    that is what tapers the shape into a drop rather than an egg.
    """
    rnd = random.Random(seed)
    pts = [(0.50 + rnd.uniform(-.06, .06), 0.03)]              # apex, off-centre
    for i in range(1, nb + 1):
        t = math.pi * (i / (nb + 1))
        ang = -math.pi / 2 + 2 * math.pi * (i / (nb + 1))
        rad = (base + swell * math.sin(t)) * (1 + rnd.uniform(-jitter, jitter))
        pts.append((0.50 + rad * math.cos(ang) * xs, cy + rad * math.sin(ang) * ys))
    seeds = [(0.50 + rnd.uniform(-.09, .09),
              0.38 + 0.28 * (j / max(1, k - 1)) + rnd.uniform(-.05, .05))
             for j in range(k)]
    n = len(pts)
    bounds = [0] + sorted(rnd.sample(range(1, n), k - 1)) + [n]
    facets = []
    for j in range(k):
        arc = (pts[bounds[j]:] + [pts[0]]) if bounds[j + 1] >= n \
            else pts[bounds[j]:bounds[j + 1] + 1]
        facets.append([seeds[j]] + arc + [seeds[(j + 1) % k]])
    facets.append(seeds)                                        # the central facet
    return facets


def render_obsidian(dest: Path, seed: int = OBSIDIAN_SEED, nb: int = 9, k: int = 5,
                    jitter: float = 0.18, base: float = 0.19, swell: float = 0.10,
                    xs: float = 1.02, ys: float = 1.42, cy: float = 0.58,
                    seam: int = 1) -> None:
    """An irregular faceted stone, roughly drop-shaped.

    Facets are **filled** and then each polygon's own outline is stroked in
    BLACK, so shared edges erase themselves and every face reads as a separate
    lit panel. Two values are load-bearing at 37x32:

    * `seam` must be **1**. At 2 the seams eat the facets and the stone reads as
      a blob rather than a cut surface — that was the first filled attempt.
    * the scale (`base`/`swell`) is set so the mark lands at ~250-300 lit
      pixels, matching the rest of the program-icon set. The first version of
      this ran 526-621 and would have shone as a bright blob beside its peers.
    """
    img = Image.new("L", (CELL_W * SS, CELL_H * SS), 0)
    d = ImageDraw.Draw(img)
    polys = [[(x * CELL_W * SS, y * CELL_H * SS) for x, y in f]
             for f in _stone_facets(seed, nb, k, jitter, base, swell, xs, ys, cy)]
    for p in polys:
        d.polygon(p, fill=255)
    for p in polys:
        d.line(p + [p[0]], fill=0, width=seam * SS, joint="curve")
    _emit(img, dest)


def ensure_obsidian(dest: Path, **kw) -> None:
    if dest.exists():
        print(f"  {dest.name}  <- committed asset (left as-is)")
        return
    render_obsidian(dest, **kw)
    print(f"  {dest.name}  <- drawn stone (seed {kw.get('seed', OBSIDIAN_SEED)})")
