...
class CommandsSubMenu():
    ...
    def buildMenu(self, parentMenu):
        cmdMenu = parentMenu.addMenu(QIcon("icons/settings.svg"), "All PolyKybd Commands")

        action = QAction(QIcon("icons/toggle_off.svg"), "Stop Idle", parent=self.parent)
        action.setData(False)
        action.triggered.connect(self.change_idle)
        cmdMenu.addAction(action)

        action = QAction(QIcon("icons/toggle_on.svg"), "Start Idle", parent=self.parent)
        action.setData(True)
        action.triggered.connect(self.change_idle)
        cmdMenu.addAction(action)

        action = QAction(QIcon("icons/delete.svg"), "Reset Overlays Buffers", parent=self.parent)
        action.triggered.connect(self.reset_overlays)
        cmdMenu.addAction(action)

        action = QAction(QIcon("icons/toggle_on.svg"), "Enable Shortcut Overlays", parent=self.parent)
        ...
        action = QAction(QIcon("icons/backlight_high_off.svg"), "Off", parent=self.parent)
        action.setData(0)
        action.triggered.connect(self.set_brightness)
        briMenu.addAction(action)

        action = QAction(QIcon("icons/backlight_low.svg"), "1%", parent=self.parent)
        action.setData(2)
        action.triggered.connect(self.set_brightness)
        briMenu.addAction(action)

        action = QAction(QIcon("icons/backlight_high.svg"), "50%", parent=self.parent)
        action.setData(25)
        action.triggered.connect(self.set_brightness)
        briMenu.addAction(action)

        action = QAction(QIcon("icons/backlight_high.svg"), "100%", parent=self.parent)
        action.setData(50)
        action.triggered.connect(self.set_brightness)
        briMenu.addAction(action)
